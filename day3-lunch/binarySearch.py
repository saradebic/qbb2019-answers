...
location = protein_coding_gene_locations_sorted[n]
for i, line in enumerate(open(sys.argv[1])):
    columns =  line.rstrip("\n").split()
    if columns[0] == "3R" and columns[2] == "gene" and "protein_coding" in line:
        if location in line:
            distance = abs(21378950-int(location))
            print("The gene closest to 3R:21,378,950 is " + str(columns[13]) + " and is " + str(distance) + " nucleotides away from the mutation.")
            print("It took " + str(count) + " iterations to find the nearest gene.")
